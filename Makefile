...
CONFIG_PKG=github.com/sipeed/jameclaw/pkg/config
...
JAMECLAW_HOME?=$(HOME)/.jameclaw
WORKSPACE_DIR?=$(JAMECLAW_HOME)/workspace
...
## build: Build the jameclaw binary for current platform
build: generate
...
## build-launcher: Build the jameclaw-launcher (web console) binary
build-launcher:
...
## build-all: Build jameclaw for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install jameclaw to system and copy builtin skills
install: build
...
## uninstall-all: Remove jameclaw and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(JAMECLAW_HOME)
	@echo "Removed workspace: $(JAMECLAW_HOME)"
...
vet: generate
	@packages="$$(go list ./...)" && \
		$(GO) vet $$(printf '%s\n' "$$packages" | grep -v '^github.com/sipeed/jameclaw/web/')
	@cd web/backend && $(WEB_GO) vet ./...

## test: Test Go code
test: generate
	@$(GO) test $$(go list ./... | grep -v github.com/sipeed/jameclaw/web/)
	@cd web && make test
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build jameclaw-agent jameclaw-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build jameclaw-agent jameclaw-gateway
...
## docker-run: Run jameclaw gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run jameclaw gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run jameclaw agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm jameclaw-agent

## docker-run-agent-full: Run jameclaw agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm jameclaw-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi jameclaw:latest jameclaw:full 2>/dev/null || true


## build-macos-app: Build JameClaw macOS .app bundle (no terminal window)
build-macos-app:
...
help:
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.jameclaw/workspace)"
