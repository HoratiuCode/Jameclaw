...
func TestGatewayRestartReturnsErrorStatusWhenReplacementFailsToStart(t *testing.T) {
	...
	invalidBinaryPath := filepath.Join(t.TempDir(), "fake-jameclaw")
	if err := os.WriteFile(invalidBinaryPath, []byte("#!/bin/sh\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	t.Setenv("JAMECLAW_BINARY", invalidBinaryPath)
...
func TestFindJameclawBinary_EnvOverride(t *testing.T) {
	...
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "jameclaw-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("JAMECLAW_BINARY", mockBinary)
...
func TestFindJameclawBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When JAMECLAW_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("JAMECLAW_BINARY", "/nonexistent/jameclaw-binary")

	got := utils.FindJameclawBinary()
	// Should not return the invalid path; falls back to "jameclaw" or another found path
	if got == "/nonexistent/jameclaw-binary" {
