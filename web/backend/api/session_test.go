...
func TestHandleSessions_IncludesTelegramJSONLStorage(t *testing.T) {
	configPath, cleanup := setupOAuthTestEnv(t)
	defer cleanup()
	...
	sessionKey := "agent:main:telegram:direct:telegram:123456"
	if err := store.AddFullMessage(nil, sessionKey, providers.Message{
		Role:    "user",
		Content: "message from telegram",
	}); err != nil {
		t.Fatalf("AddFullMessage(user) error = %v", err)
	}
	if err := store.AddFullMessage(nil, sessionKey, providers.Message{
	...
	if items[0].AgentID != "main" || items[0].Channel != "telegram" || items[0].ChatType != "direct" || items[0].ChatID != "telegram:123456" {
		t.Fatalf("items[0] identity = agent:%q channel:%q chat_type:%q chat_id:%q",
			items[0].AgentID, items[0].Channel, items[0].ChatType, items[0].ChatID)
	}
	if items[0].Preview != "message from telegram" {
		t.Fatalf("items[0].Preview = %q", items[0].Preview)
	}
	...
	if detail.ID != sessionKey || len(detail.Messages) != 2 {
		t.Fatalf("detail = %#v, want id %q with 2 messages", detail, sessionKey)
	}
}
...
func TestHandleDeleteSession_TelegramJSONLStorage(t *testing.T) {
	configPath, cleanup := setupOAuthTestEnv(t)
	defer cleanup()
	...
	if err := store.AddFullMessage(nil, sessionKey, providers.Message{
		Role:    "user",
		Content: "delete telegram session",
	}); err != nil {
		t.Fatalf("AddFullMessage() error = %v", err)
	}
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodDelete, "/api/sessions/"+sessionKey, nil)
	mux.ServeHTTP(rec, req)
