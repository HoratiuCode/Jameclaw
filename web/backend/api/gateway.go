...
func (h *Handler) startGatewayLocked(initialStatus string, existingPid int) (int, error) {
	...
	// Start new process
	// Locate the jameclaw executable
	...
	pid = cmd.Process.Pid
	logger.InfoC("gateway", fmt.Sprintf("Started jameclaw gateway (PID: %d) from %s", pid, execPath))
...
// handleGatewayStart starts the jameclaw gateway subprocess.
//
