...
func setupJameEnabledEnv(t *testing.T) (string, func()) {
	...
	oldJameHome := os.Getenv("JAMECLAW_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("JAMECLAW_HOME", filepath.Join(tmp, ".jameclaw")); err != nil {
		t.Fatalf("set JAMECLAW_HOME: %v", err)
	}
	...
	cleanup := func() {
		...
		if oldJameHome == "" {
			_ = os.Unsetenv("JAMECLAW_HOME")
		} else {
			_ = os.Setenv("JAMECLAW_HOME", oldJameHome)
		}
	}
