...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("JAMECLAW_BUILTIN_SKILLS")
	if err := os.Setenv("JAMECLAW_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(JAMECLAW_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("JAMECLAW_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("JAMECLAW_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
