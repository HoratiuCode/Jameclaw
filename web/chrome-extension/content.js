...
async function runBrowserCommand(command) {
  try {
    ...
    switch (command.action) {
      ...
      case "type": {
        const element = document.querySelector(args.selector)
        if (!(element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement || element instanceof HTMLSelectElement)) throw new Error(`Selector is not a form field: ${args.selector}`)
        element.focus(); element.value = String(args.text || ""); element.dispatchEvent(new Event("input", { bubbles: true })); element.dispatchEvent(new Event("change", { bubbles: true })); return { content: `Typed text into ${args.selector}` }
      }
