...
import (
	...
	"github.com/sipeed/jameclaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to jameclaw",
		Args:  cobra.NoArgs,
		Example: `  jameclaw migrate
  jameclaw migrate --from openclaw
  jameclaw migrate --dry-run
  jameclaw migrate --refresh
  jameclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.jameclaw)")
