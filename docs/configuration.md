...
Config file: `~/.jameclaw/config.json`

### Environment Variables

You can override default paths using environment variables. This is useful for portable installations, containerized deployments, or running jameclaw as a system service. These variables are independent and control different paths.

| Variable          | Description                                                                                                                             | Default Path              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `JAMECLAW_CONFIG` | Overrides the path to the configuration file. This directly tells jameclaw which `config.json` to load, ignoring all other locations. | `~/.jameclaw/config.json` |
| `JAMECLAW_HOME`   | Overrides the root directory for jameclaw data. This changes the default location of the `workspace` and other data directories.          | `~/.jameclaw`             |

**Examples:**

```bash
# Run jameclaw using a specific config file
# The workspace path will be read from within that config file
JAMECLAW_CONFIG=/etc/jameclaw/production.json jameclaw gateway

# Run jameclaw with all its data stored in /opt/jameclaw
# Config will be loaded from the default ~/.jameclaw/config.json
# Workspace will be created at /opt/jameclaw/workspace
JAMECLAW_HOME=/opt/jameclaw jameclaw agent

# Use both for a fully customized setup
JAMECLAW_HOME=/srv/jameclaw JAMECLAW_CONFIG=/srv/jameclaw/main.json jameclaw gateway
```

### Workspace Layout

JameClaw stores data in your configured workspace (default: `~/.jameclaw/workspace`):
...
1. `~/.jameclaw/workspace/skills` (workspace)
2. `~/.jameclaw/skills` (global)
...
```bash
export JAMECLAW_BUILTIN_SKILLS=/path/to/skills
```
...
When multiple bindings exist, JameClaw resolves in this order:
...
If a binding points to a missing `agent_id`, JameClaw falls back to the default agent.

#### How matching works (step-by-step)

1. JameClaw first filters bindings by `match.channel` (must equal current channel).
...
JameClaw runs in a sandboxed environment by default. The agent can only access files and execute commands within the configured workspace.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.jameclaw/workspace` | Working directory for the agent           |
...
| `create_pdf`  | Create and send PDF files | Only writes within workspace            |
...
The exec safety guard only inspects the command line JameClaw launches directly. It does not recursively inspect child
...
* Run JameClaw inside a container or VM if you need stronger isolation than the built-in guard provides.
...
```bash
export JAMECLAW_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
JameClaw can perform periodic tasks automatically. Create a `HEARTBEAT.md` file in your workspace:
...
* `JAMECLAW_HEARTBEAT_ENABLED=false` to disable
* `JAMECLAW_HEARTBEAT_INTERVAL=60` to change interval
...
> **What's New?** JameClaw now uses a **model-centric** configuration approach. Simply specify `vendor/model` format (e.g., `zhipu/glm-4.7`) to add new providers — **zero code changes required!**
...
| **VolcEngine (Doubao)** | `volcengine/`     | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Get Key](https://www.volcengine.com/activity/codingplan?utm_campaign=JameClaw&utm_content=JameClaw&utm_medium=devrel&utm_source=OWO&utm_term=JameClaw) |
...
> Run `jameclaw auth login --provider anthropic` to paste your API token.
...
JameClaw strips only the outer `litellm/` prefix before sending the request, so `litellm/lite-gpt4` sends `lite-gpt4`, while `litellm/openai/gpt-4o` sends `openai/gpt-4o`.

</details>

#### Load Balancing

Configure multiple endpoints for the same model name — JameClaw will automatically round-robin between them:
...
JameClaw routes providers by protocol family:
...
JameClaw supports cron-style scheduled tasks via the `cron` tool. The agent can set, list, and cancel reminders or recurring jobs that trigger at specified times.
