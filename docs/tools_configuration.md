...
JameClaw's tools configuration is located in the `tools` field of `config.json`.
...
Before tool results are sent to the LLM, JameClaw can filter sensitive values (API keys, tokens, secrets) from the output. This prevents the LLM from seeing its own credentials.
...
```bash
JAMECLAW_TOOLS_EXEC_ENABLED=false
```
...
By default, JameClaw blocks the following dangerous commands:
...
The exec guard only validates the top-level command sent to JameClaw. It does **not** recursively inspect child
...
All configuration options can be overridden via environment variables with the format `JAMECLAW_TOOLS_<SECTION>_<KEY>`:

For example:

- `JAMECLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `JAMECLAW_TOOLS_EXEC_ENABLED=false`
- `JAMECLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `JAMECLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `JAMECLAW_TOOLS_MCP_ENABLED=true`
