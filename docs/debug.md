...
## Starting JameClaw in Debug Mode

To get detailed information about what the agent is doing (LLM requests, tool calls, message routing), you can start the JameClaw gateway with the debug flag:

```bash
jameclaw gateway --debug
# or
jameclaw gateway -d
```
...
By default, JameClaw truncates very long strings (such as the *System Prompt* or large JSON output results) in the debug logs to keep the console readable.
...
```bash
jameclaw gateway --debug --no-truncate
...
[...] [INFO] agent: Tool call: web_search({"query":"jameclaw release notes"}) {tool=web_search, iteration=1}
```
...
```bash
JAMECLAW_AGENTS_DEFAULTS_TOOL_FEEDBACK_ENABLED=true
JAMECLAW_AGENTS_DEFAULTS_TOOL_FEEDBACK_MAX_ARGS_LENGTH=300
```
