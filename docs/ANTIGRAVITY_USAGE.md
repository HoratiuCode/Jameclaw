# Using Antigravity Provider in JameClaw

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in JameClaw.
...
```bash
jameclaw auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where JameClaw is waiting.

JameClaw will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.jameclaw/config.json` or override it via the CLI:

```bash
# Override for a single command
jameclaw agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Environment Variables**:
    *   `JAMECLAW_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Authentication persistence**: 
    If you've logged in locally, you can copy your credentials to the server:
    ```bash
    scp ~/.jameclaw/auth.json user@your-server:~/.jameclaw/
    ```
...
*   **429 Rate Limit**: Antigravity has strict quotas. JameClaw will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `jameclaw auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
