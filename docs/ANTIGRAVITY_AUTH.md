...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in JameClaw.
...
10. [Creating a New Provider in JameClaw](#creating-a-new-provider-in-jameclaw)
...
const antigravityPlugin = {
  ...
  register(api: JameClawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<JameClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.25
- JameClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.jameclaw/auth.json`:
...
## Creating a New Provider in JameClaw

JameClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/jameclaw/internal/auth/helpers.go`:
...
jameclaw auth login --provider your-provider

# List models (for Antigravity)
jameclaw auth models

# Start the gateway
jameclaw gateway

# Run an agent with a specific model
jameclaw agent -m "Hello" --model your-model
```
...
export JAMECLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export JAMECLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
- **Source Files:**
  ...
  - `pkg/auth/store.go` - Auth credential storage (`~/.jameclaw/auth.json`)
  - `pkg/providers/factory.go` - Provider factory and protocol routing
  - `pkg/providers/types.go` - Provider interface definitions
  - `cmd/jameclaw/internal/auth/helpers.go` - Auth CLI commands
...
### "Token expired"
- Refresh OAuth tokens: `jameclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.jameclaw/auth.json`
- Re-run `jameclaw auth login --provider antigravity`
