...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("jameclaw")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@jameclaw hello", want: true},
		{text: "hi @jameclaw:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bjameclaw\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@jameclaw:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@jameclaw:matrix.org hello", want: "hello"},
		{in: "@jameclaw, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@jameclaw:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@jameclaw:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@jameclaw:matrix.org">JameClaw</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40jameclaw%3Amatrix.org">JameClaw</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@jameclaw:matrix.test"), "")
	if err != nil {
