// JameClaw - Ultra-lightweight personal AI agent
// License: MIT
//
// Copyright (c) 2026 JameClaw contributors

package config

// Runtime environment variable keys for the jameclaw process.
// These control the location of files and binaries at runtime and are read
// directly via os.Getenv / os.LookupEnv. All jameclaw-specific keys use the
// JAMECLAW_ prefix. Reference these constants instead of inline string
// literals to keep all supported knobs visible in one place and to prevent
// typos.
const (
	// EnvHome overrides the base directory for all jameclaw data
	// (config, workspace, skills, auth store, …).
	// Default: ~/.jameclaw
	EnvHome = "JAMECLAW_HOME"

	// EnvConfig overrides the full path to the JSON config file.
	// Default: $JAMECLAW_HOME/config.json
	EnvConfig = "JAMECLAW_CONFIG"

	// EnvBuiltinSkills overrides the directory from which built-in
	// skills are loaded.
	// Default: <cwd>/skills
	EnvBuiltinSkills = "JAMECLAW_BUILTIN_SKILLS"

	// EnvBinary overrides the path to the jameclaw executable.
	// Used by the web launcher when spawning the gateway subprocess.
	// Default: resolved from the same directory as the current executable.
	EnvBinary = "JAMECLAW_BINARY"

	// EnvGatewayHost overrides the host address for the gateway server.
	// Default: "127.0.0.1"
	EnvGatewayHost = "JAMECLAW_GATEWAY_HOST"
)
