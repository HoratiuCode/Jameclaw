...
```bash
cp security.example.yml ~/.jameclaw/.security.yml
```

### Step 2: Fill in your actual values

Edit `~/.jameclaw/.security.yml` and replace placeholder values with your actual API keys and tokens.

### Step 3: Update config.json

Replace sensitive values in `~/.jameclaw/config.json` with `ref:` references:
...
Restart JameClaw and verify it loads correctly:
```bash
jameclaw --version
```
...
2. **Set file permissions**: `chmod 600 ~/.jameclaw/.security.yml`
...
**For models:**
```bash
export JAMECLAW_MODEL_LIST_GPT-5.4_API_KEY="sk-from-env"
```

**For channels:**
```bash
export JAMECLAW_CHANNELS_TELEGRAM_TOKEN="token-from-env"
```

**For web tools:**
```bash
export JAMECLAW_WEB_BRAVE_API_KEY="key-from-env"
```

Environment variables follow this pattern: `JAMECLAW_<SECTION>_<KEY1>_<KEY2>_<FIELD>` with dots replaced by underscores and converted to uppercase.
