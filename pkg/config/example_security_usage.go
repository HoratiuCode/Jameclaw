...
```bash
chmod 600 ~/.jameclaw/security.yml
```
...
```bash
cp ~/.jameclaw/config.json ~/.jameclaw/config.json.backup
```
...
Edit ~/.jameclaw/.security.yml and replace placeholders with your actual keys.
...
```bash
rm ~/.jameclaw/config.json.backup
```
