...
func TestSaveStore_FilePermissions(t *testing.T) {
	...
	_, err := cs.AddJob("test", CronSchedule{Kind: "every", EveryMS: int64Ptr(60000)}, "hello", false, false, "cli", "direct")
	if err != nil {
...
func TestCronService_CRUD(t *testing.T) {
	...
	job, err := cs.AddJob("Task1", CronSchedule{Kind: "at", AtMS: &at}, "msg", true, true, "ch", "to")
	if err != nil || job.ID == "" {
		t.Fatalf("AddJob failed: %v", err)
	}
...
func TestCronService_ExecutionFlow(t *testing.T) {
	...
	job, _ := cs.AddJob("FastJob", CronSchedule{Kind: "at", AtMS: &target}, "", false, false, "", "")
...
func TestCronService_PersistenceIntegrity(t *testing.T) {
	...
	cs1.AddJob("PersistMe", CronSchedule{Kind: "at", AtMS: &at}, "payload", true, true, "ch1", "")
	...
	for i := range workers {
		go func(id int) {
			...
			for j := range iterations {
				at := time.Now().Add(time.Hour).UnixMilli()
				cs.AddJob(fmt.Sprintf("Job-%d-%d", id, j), CronSchedule{Kind: "at", AtMS: &at}, "", false, false, "", "")
