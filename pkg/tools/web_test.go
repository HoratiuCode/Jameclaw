...
// with cf-mitigated: challenge triggers a retry using the honest jameclaw User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest jameclaw user agent
	if !strings.Contains(receivedUAs[1], "jameclaw") {
		t.Errorf("retry request UA = %q, want it to contain 'jameclaw'", receivedUAs[1])
	}
}
