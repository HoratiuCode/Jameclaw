...
// openclaw home directory when migrating from openclaw to jameclaw.
// Default: ~/.openclaw
...
func (o *OpenclawHandler) ExecuteConfigMigration(srcConfigPath, dstConfigPath string) error {
	...
	jameCfg, warnings, err := openclawCfg.ConvertToJameClaw(o.opts.SourceHome)
	if err != nil {
		return err
	}
...
func rewriteWorkspacePath(path string) string {
	path = strings.Replace(path, ".openclaw", ".jameclaw", 1)
	return path
}
