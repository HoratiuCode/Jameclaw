...
func TestProcessHook_HelperProcess(t *testing.T) {
	if os.Getenv("JAMECLAW_HOOK_HELPER") != "1" {
		return
	}
...
func processHookHelperEnv(mode, eventLog string) []string {
	env := []string{
		"JAMECLAW_HOOK_HELPER=1",
		"JAMECLAW_HOOK_MODE=" + mode,
	}
	if eventLog != "" {
		env = append(env, "JAMECLAW_HOOK_EVENT_LOG="+eventLog)
	}
	return env
}
...
func runProcessHookHelper() error {
	mode := os.Getenv("JAMECLAW_HOOK_MODE")
	eventLog := os.Getenv("JAMECLAW_HOOK_EVENT_LOG")
