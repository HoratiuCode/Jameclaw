...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via JAMECLAW_KEY_PASSPHRASE
...
//  2. JAMECLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/jameclaw_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "JAMECLAW_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading JAMECLAW_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "JAMECLAW_SSH_KEY_PATH"

// jameclawHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const jameclawHome = "JAMECLAW_HOME"

const (
	fileScheme = "file://"
	encScheme  = "enc://"
	hkdfInfo   = "jameclaw-credential-v1"
	saltLen    = 16
	nonceLen   = 12
	keyLen     = 32
)
...
//
// passphrase is required (JAMECLAW_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// JAMECLAW_SSH_KEY_PATH env var or ~/.ssh/jameclaw_ed25519.key.
...
//   - exact match with JAMECLAW_SSH_KEY_PATH env var
//   - within the JAMECLAW_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with JAMECLAW_SSH_KEY_PATH.
	...
	// Within JAMECLAW_HOME.
	if jameHome := os.Getenv(jameclawHome); jameHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="jameclaw-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set JAMECLAW_SSH_KEY_PATH or place key at ~/.ssh/jameclaw_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (JAMECLAW_SSH_KEY_PATH, JAMECLAW_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. JAMECLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/jameclaw_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the jameclaw-specific SSH key path if it exists.
